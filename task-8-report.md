...
## Changes
- Added `antigravity_oauth.dart` with OAuth code exchange, PKCE/state loopback handoff, production Windows external-browser launcher, transport-only prod/daily provisioning retry, onboarding identity checks, strict deep quota schema validation, 429/5xx transient handling, schema-change tombstones, legacy `fetchAvailableModels` + `retrieveUserQuota` fallback with model quota merging, and offline refresh-token exchange.
...
- Added focused fake HTTP coverage for account isolation, mismatches, onboarding, quota parsing, and required regression behavior.
