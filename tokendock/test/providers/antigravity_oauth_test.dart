...
void main() {
  test('remote provider keeps account tokens isolated', () async {
    ...
    final snapshot = await provider.fetch(
      connection,
      (await store.read(connection.credentialRef))!,
    );
  ...
  test(
    'provider routes opted-in local sources through the local reader',
    () async {
      final local = AntigravityProvider(
        localReader: AntigravityLocalReader(
      ...
      expect(
        (await local.fetch(localConnection, '')).quotas.single.remaining,
        0.25,
      );
  ...
  test(
    'provider keeps remote source as the default and explicit mode',
    () async {
      final http = _Http([
        _Response(
          200,
      ...
      expect(
        (await provider.fetch(
          _connection(
            'explicit',
            providerData: jsonEncode({'source': 'remote'}),
          ),
          secret,
        )).quotas.single.remaining,
        0.4,
  ...
  test('quota schema rejects malformed remaining and reset but accepts reset-only bucket', () async {
    ...
    final malformed = await provider.fetch(
      _connection('a'),
      jsonEncode({
        'accessToken': 'a',
        'identityKey': 'a@example.com|acct-a',
    ...
    final valid = AntigravityOAuthProvider(
      http: _Http([
        _Response(
          200,
          jsonEncode({
    ...
    final resetOnly = await valid.fetch(
      _connection('a'),
      jsonEncode({
        'accessToken': 'a',
        'identityKey': 'a@example.com|acct-a',
    ...
    final badReset = AntigravityOAuthProvider(
      http: _Http([
        _Response(
          200,
          jsonEncode({
  ...
  test('refresh preserves refresh token when Google omits rotation', () async {
    ...
    final secret = await provider.refresh(
      jsonEncode({
        'accessToken': 'old',
  ...
  test('refresh invalid_grant is definitive', () async {
    ...
    await expectLater(
      provider.refresh(jsonEncode({'refreshToken': 'revoked'})),
  ...
  test('test refreshes an expiring refreshable credential once before fetch', () async {
    ...
    expect(http.requests.map((request) => request.uri), [
      ...
      Uri.parse(
        '${AntigravityOAuthProvider.prodHost}/v1internal:retrieveUserQuotaSummary',
      ),
    ]);
  });

  test(
    'rotated refresh-token reuse revokes the local pair and requires relogin',
    () async {
      final http = _RecordingHttp([
        _Response(
          200,
      ...
      await expectLater(
        ...
        throwsA(
          predicate<Object>(
            (error) => error.toString().contains('invalid_grant'),
          ),
        ),
      );
      expect(
  ...
  test('two accounts keep independent OAuth secrets', () async {
    final store = _Store();
    final a = AntigravityOAuthProvider(
      http: _Http([
        _Response(
          200,
          jsonEncode({
    ...
    final b = AntigravityOAuthProvider(
      http: _Http([
        _Response(
          200,
          jsonEncode({
        ...
        _Response(
          ...
          jsonEncode({
            'response': {
              ...
              'accountId': 'acct-b',
              'currentTier': {'id': 'free'},
              'cloudaicompanionProject': 'project-b',
            },
          }),
        ),
      ]),
  ...
  test(
    'loginWithLoopback launches browser, delivers callback, and closes session',
    () async {
      final store = _Store();
      final http = _RecordingHttp([
        _Response(
      ...
      await expectLater(
        client
            .getUrl(Uri.parse(opened!.queryParameters['redirect_uri']!))
  ...
  test(
    'loginWithLoopback reports cancellation while waiting for callback',
    () async {
      ...
      final login = provider.loginWithLoopback(
        _connection('cancelled'),
        cancellation: cancellation.future,
      );
      await launched.future;
      cancellation.complete();

      await expectLater(login, throwsA(isA<AntigravityLoginCancelled>()));
      expect(opened, isNotNull);
    },
  );

  test(
    'loadCodeAssist falls back from production transport failure to daily',
    () async {
      final http = _HostHttp({
        'https://oauth2.googleapis.com/token': _Response(
          200,
          jsonEncode({
            'access_token': 'a',
            'refresh_token': 'r',
  ...
  test(
    'token endpoint requests are form-encoded while RPC requests stay JSON',
    () async {
      final http = _RecordingHttp([
        _Response(
          200,
      ...
      expect(
        Uri.splitQueryString(http.requests.first.body)['code_verifier'],
        'verifier',
      );
  ...
  test(
    'Retry-After seconds and HTTP-date become the first retry floor',
    () async {
      final delays = <Duration>[];
      final secret = jsonEncode({
        'accessToken': 'a',
      ...
      expect(
        second.cooldownUntil!.difference(second.fetchedAt).inSeconds,
        inInclusiveRange(239, 241),
      );
    },
  );
}
