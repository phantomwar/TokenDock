...
- `flutter test --no-pub`: **465/465 passed** (`9511321`), re-confirmed on three consecutive runs. The 366-test gate was met on three consecutive runs at `9ae626f`; 379, 386, 389, 399, 409, 410, 417 and 430 were each re-confirmed on three consecutive runs before the next change. Baseline before the 2026-09-26 session was 235.
- **Providers:** OpenRouter, Antigravity, MiniMax. MiniMax is credential-verified against `GET https://api.minimax.io/v1/models` (401 on a bad key) but publishes no usage API, so it shows connection health only. OpenCode Zen/Go are intentionally absent — their `/models` endpoints return 200 for an invalid key, so they cannot support a test-before-save gate; see `OpenCodeSupport` in `lib/providers/provider_registry.dart`.
