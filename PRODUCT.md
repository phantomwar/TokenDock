...
- `Migration001` creates `auth_type`; `Migration003` adds `identity_key` and `provider_data` and sets SQLite `user_version = 3`. `AppDatabase.open` runs `Migration001`–`Migration003`. Migrations now stamp `user_version` inside the DDL transaction and gate `ALTER TABLE` on `PRAGMA table_info`, so an interrupted migration is resumable instead of failing on `duplicate column name`.
...
- Current evidence at `9ae626f`: `flutter test --no-pub` passes **366 tests** on three consecutive runs; `flutter analyze` reports **0 errors, 0 warnings, 31 informational diagnostics**. Prior baseline was 235 tests and 33 diagnostics. Individual commits are verified in isolation.
