...
- Current evidence at `6b93b7b`: `flutter test --no-pub` passes **379 tests**; the 366-test gate was met on three consecutive runs at `9ae626f` and the 13 tests added since are verified in isolation. `flutter analyze` reports **0 errors, 0 warnings, 31 informational diagnostics** — the same count as before this change, with none in the touched files. Prior baseline was 235 tests and 33 diagnostics.
