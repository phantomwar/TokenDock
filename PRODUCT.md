...
- **`windows/runner/Runner.rc` still ships the Flutter template defaults**, so `CompanyName` is `com.example` — a domain reserved for examples (RFC 2606) — and `ProductName` is the lowercase project name. Consequences: the secure storage lives under `%APPDATA%\com.example\tokendock\`, which any other Flutter app shipping the same default would also use, and Windows' file properties show `com.example` as the publisher. Correcting the name would orphan every already-stored credential, because the plugin derives the path from it, so it needs a migration rather than a one-line edit.
