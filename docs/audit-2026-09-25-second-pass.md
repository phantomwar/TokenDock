...
| C-19 | P2 | Tipografia não bate com a spec | `theme.dart:172-199` | ✅ `b853186` |
...
| C-26 | P2 | `redactHeaders`/`redactUrl` mortas | `log_redaction.dart:72,81` | ✅ `45aad6c` |
| C-27 | P2 | `_credential` duplicado | `antigravity_oauth.dart:808,861` | ✅ `45aad6c` |
| C-28 | P2 | `AuthKind` vs strings sem validação | `app_state.dart:432,810` | ✅ `14f6979` |
| C-29 | P2 | `deleteSync` mascara timeout | `antigravity_local.dart:543-545` | ✅ `f43e27b` |
| C-30 | P2 | Segredo cru retido no widget | `connections_screen.dart:376-385` | ✅ `f43e27b` |
| C-31 | P3 | "No key cap" → "Unavailable" | `quota_row.dart:21` | ✅ `4faea73` |
| C-32 | P3 | `AppCard`/`SectionHeader` mortos | `app_card.dart:10` | ✅ `14f6979` |
| C-33 | P3 | `defaultRefreshIntervalMinutes` duplicado | `settings_repository.dart:9,42` | ✅ `14f6979` |
| C-34 | P3 | Drift de documentação (8 itens) | ver tabela §2 P3 | ✅ `75b7ef5` |
| C-35 | P3 | 1 asserção de contraste; 2 invariantes sem teste | `theme_test.dart` | ✅ `757aa09` |
| C-36 | P3 | Flake: sleeps reais + sockets de 250ms | `refresh_service_test.dart` | ✅ `757aa09` |

### 4.1 Consolidado

| Situação | Qtd | IDs |
|---|---|---|
| ✅ Fechado | 36 | C-01 … C-36 (todos) |
| ⬜ Aberto | 0 | — |

**Todos os 36 achados desta auditoria estão fechados**, incluindo os 5 P0, os
11 P1 e os 3 P2 de maior valor (C-22, C-23, C-24). Cada linha da tabela acima
tem o commit que a fechou.

**O que C-19 revelou, e porque o plano adiou-o com razão.** A rampa tipográfica
tinha dois conflitos que uma edição mecânica teria perdido. O countdown
partilhava `quotaStyle` com a figura da quota: aos 20px semibold a linha
"resets in 2h 15m" gritava tão alto como o número que anota, exactamente na
densidade compacta, onde a figura já disputa espaço. Passou a ter passo próprio.
E a rampa tinha 18px e 22px sem token nenhum, expressos como `copyWith` na
call site — que é como derivou sem que um teste pudesse vê-la.

**Sobre o teste do layout a 360px.** A janela real é 360x600, o que deixa ~326px
de conteúdo e cai em densidade compacta. O teste a esse tamanho passa, mas
**não fiscaliza o tamanho da figura**: verifiquei subindo o passo para 40px e
continuou a passar, porque a figura é um `Text` que quebra linha. O comentário
no teste diz isso, em vez de fingir uma guarda que não existe.
...
impossível de usar: ele dispararia *antes* das migrations manuais e deixaria a
enforcement ativa sobre o rebuild que a FK exige. Medido no SQLite empacotado:
um rebuild com um único órfão pré-existente falha com erro 787, e um
`foreign_keys = OFF` in-transaction não resgatá-lo. Ligar o pragma em
`TestDatabase` expôs dois fixtures que guardavam quota de conexões que nunca
existiram — estados que a base agora recusa corretamente.
