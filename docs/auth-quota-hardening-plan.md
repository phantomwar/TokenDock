...
- Login adaptation implemented in the current worktree (`c0acaef`): `AuthKind` now distinguishes `apiKey`, `oauth`, `structuredBearer`, and `none`; the registry exposes `openrouter` (`apiKey`) and `antigravity` (`oauth`). `Migration001` creates `auth_type`; `Migration003` adds `identity_key` and `provider_data` and sets SQLite `user_version = 3`.
- OpenRouter hardening remains implemented: classification 401/402/403/429/503; `Retry-After` in seconds or HTTP-date for 429/503 and for 402 only with `metadata.limit_source=openrouter_in_flight_budget`; per-connection cooldown/health persistence; cache preservation; current-secret redaction; and cooldown reset on success.
- Antigravity Appendix A is implemented as a backend/test-only, opt-in local read-only `AntigravityLocalReader`: language-server quota sources, `agy` CLI fallback with version/output/time bounds, Gemini and Claude/GPT pool mapping, availability-only handling, and selected-account mismatch rejection. The Connections UI is not wired to select this source.
- Antigravity Appendix B is implemented as a backend/test-only per-account remote `AntigravityOAuthProvider`: loopback browser + PKCE login, isolated refreshable OAuth secrets, `loadCodeAssist`/`onboardUser` handling, selected-account guard, quota retrieval, and schema-change handling that preserves cached values. The Connections UI is not wired to launch this login flow.
- Refreshable credentials and OAuth single-flight coordination are implemented for the remote provider backend; the SQLite metadata supports per-account identity and provider state. Polling remains fixed-cadence and no automatic account fallback is implemented.
- Current evidence: `flutter test --no-pub` 165/165 passed. `flutter analyze` exited nonzero with 0 errors, 0 warnings, and 14 informational diagnostics; six pre-existing `prefer_initializing_formals` diagnostics remain in `RefreshService`, with the remaining diagnostics in new Antigravity/provider/test code. Windows integration verification remains blocked by symlink support/Developer Mode.
- MiniMax remains blocked because the official FAQ documents `GET https://www.minimaxi.com/v1/token_plan/remains` but not its response schema. OpenCode Go remains deferred because it has no verified public subscription-quota API. The three-provider product target is still unmet because only OpenRouter and Antigravity are registered.
