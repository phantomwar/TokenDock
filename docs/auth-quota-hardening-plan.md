...
- Login adaptation implemented in the current worktree (`c0acaef`): `AuthKind` now distinguishes `apiKey`, `oauth`, `structuredBearer`, and `none`; the registry exposes `openrouter` (`apiKey`) and `antigravity` (`oauth`). `Migration003` adds `auth_type`, `identity_key`, and `provider_data` to `connections` and sets SQLite `user_version = 3`.
...
- Antigravity Appendix A is implemented as the opt-in local read-only `AntigravityLocalReader`: language-server quota sources, `agy` CLI fallback with version/output/time bounds, Gemini and Claude/GPT pool mapping, availability-only handling, and selected-account mismatch rejection.
- Antigravity Appendix B is implemented as the default per-account remote `AntigravityOAuthProvider`: loopback browser + PKCE login, isolated refreshable OAuth secrets, `loadCodeAssist`/`onboardUser` handling, selected-account guard, quota retrieval, and schema-change handling that preserves cached values.
- Refreshable credentials and single-flight token refresh are wired for OAuth; the SQLite metadata supports per-account identity and provider state. Polling remains fixed-cadence and no automatic account fallback is implemented.
- Current evidence: `flutter test --no-pub` 165/165 passed. `flutter analyze` reports 0 errors, 2 warnings, and 14 informational diagnostics; six pre-existing `prefer_initializing_formals` diagnostics remain in `RefreshService`, with the remaining diagnostics in new Antigravity/provider/test code. Windows integration verification remains blocked by symlink support/Developer Mode.
